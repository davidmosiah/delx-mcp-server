...
assert.match(skill, /call health/);


const bin = join(root, "bin/delx-mcp-server.js");
function run(args) {
...
const result = await run(["call", "health", "--json", "{}"]);
...
console.log(JSON.stringify({ ok: true, suite: "skill-surface", tool: "health" }, null, 2));
